...
export const generateWebsite = async(req,res)=>{
    try{
        ...
        return res.status(201).json({message: "Website generated successfully", websiteId: website._id,remainingCredits: user.credits});
    }
    catch(err){
...
export const changes = async(req,res)=>{
    try{
        ...
        return res.status(201).json({message: parsed.message,code: parsed.code,remainingCredits: user.credits});

    }
    catch(err){
        return res.status(500).json({message: `Changes error ${err.message}`})
    }
}
...
export const getAll = async(req,res)=>{
    try{
        ...
        return res.status(500).json({message: `Get all websites error ${err.message}`})
    }
}
...
export const deploy = async(req,res)=>{
    try{
        ...
        if(!website.slug){
            website.slug = website.title.toLowerCase().replace(/[^a-z0-9]/g,"").slice(0,50)+website._id.toString().slice(-5)
        }
        ...
        return res.status(500).json({message: `Deploy error ${err.message}`})
    }
}
...
export const getBySlug = async(req,res)=>{
    try{
        ...
        return res.status(500).json({message: `Get by slug error ${err.message}`})
    }
}
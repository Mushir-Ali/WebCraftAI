...
function Dashboard() {
    ...
    const handleDeploy = async (id) => {
        try {
            ...
            setWebsites((prev)=> 
            prev.map((w)=>
            w._id === id ? {...w, deployed: true, deployedUrl: result.data.url} : w));
        }
        catch (err) {
            console.log(err);
        }
                        ...
                        {websites.map((w, i) => {
                            ...
                            >
                                ...
                                <div className='p-5 flex flex-col gap-4 flex-1'>
                                    ...
                                    {!w.deployed ? (
                                        <button className='mt-auto flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-indigo-500 to-purple-500 hover:scale-105 transition' onClick={(e) => {
                                            e.stopPropagation();
                                            handleDeploy(w._id)
                                        ...
                                        <motion.button
                                            ...
                                            className={`mt-auto flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${copied ? "bg-emerald-500/20 text-emerald-400 border bprder-emerald-500/30" : "bg-white/10 hover:bg-white/20 border border-white/10"}`}
                                        >
                                        {copied ? (
                                            <>
                                            <Check size={14}/>
